...
/**
 * An action that displays a scan-two command editor dialog.
 * @author <A HREF="mailto:keith.sheppard@jax.org">Keith Sheppard</A>
 */
public class ScanTwoAction extends AbstractAction implements ProjectChangeListener
{
    /**
     * every {@link java.io.Serializable} is supposed to have one of these
     */
    private static final long serialVersionUID = 1621885537494452201L;
    ...
    private final ActionListener scanApprovedListener = new ActionListener()
    {
        public void actionPerformed(ActionEvent e)
        {
            ...
            Thread evaluateCommandThread = new Thread()
            {
                ...
                public void run()
                {
                    ScanTwoAction.this.evaluateScanCommand(
                            scanCommandBuilder);
                }
            };
     ...
     * Constructor
     */
    public ScanTwoAction()
    {
        this(null);
    }
    
    /**
     * Create a scantwo action for the given cross.
     * @param selectedCross
     *          the cross
     */
    public ScanTwoAction(Cross selectedCross)
    {
        super("Run Two QTL Genome Scan ...");
     ...
     */
    private void evaluateScanCommand(ScanCommandBuilder scanCommandBuilder)
    {
        RInterface rInterface = RInterfaceFactory.getRInterfaceInstance();
        rInterface.insertComment(
                "running scantwo on cross: " +
        ...
        {
            rInterface.insertComment(
                    "running scantwo permutations (this can take a while)");
        ...
        SwingUtilities.invokeLater(
                new Runnable()
                {
                    public void run()
                    {
                        ...
                        Cross selectedCross = ScanTwoAction.this.selectedCross;
                        if(selectedCross == null)
                        {
                        ...
                        scanDialog.addActionListener(
                                ScanTwoAction.this.scanApprovedListener);
    ...
    public void projectChangeOccurred(ProjectManager projectManager)
    {
        ...
        SwingUtilities.invokeLater(new Runnable()
        {
            public void run()
            {
                ScanTwoAction.this.setEnabled(anyCrosses);
            }
        });
    }
}
