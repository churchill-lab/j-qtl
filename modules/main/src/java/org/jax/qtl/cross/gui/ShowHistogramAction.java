...
/**
 * Action for showing a histogram
 * @author <A HREF="mailto:keith.sheppard@jax.org">Keith Sheppard</A>
 */
public class ShowHistogramAction extends AbstractAction implements ProjectChangeListener
{
    /**
     * every {@link java.io.Serializable} is supposed to have one of these
     */
    private static final long serialVersionUID = 1562821703793487531L;
     ...
     *          the cross
     */
    public ShowHistogramAction(Cross selectedCross)
    {
        super("Histogram ...");
        ...
        SwingUtilities.invokeLater(
                new Runnable()
                {
                    public void run()
                    {
                        ...
                        Cross selectedCross = ShowHistogramAction.this.selectedCross;
                        if(selectedCross == null)
                        {
                        ...
                        ShowHistogramDialog showHistogramDialog =
                            new ShowHistogramDialog(
                                    QTL.getInstance().getApplicationFrame(),
                                    selectedCross,
                                    crosses);
                        showHistogramDialog.setVisible(true);
                    }
                });
    }
    ...
    public void projectChangeOccurred(ProjectManager projectManager)
    {
        ...
        SwingUtilities.invokeLater(new Runnable()
        {
            public void run()
            {
                ShowHistogramAction.this.setEnabled(anyCrosses);
            }
        });
    }
}
